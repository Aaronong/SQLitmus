...
function randomEssayGenerator(randNum, [maxLength]) {
  const seed = randNum * Number.MAX_SAFE_INTEGER;
  faker.seed(seed);
  const result = faker.lorem.paragraphs();
  return doubleQuote(result.length > maxLength ? result.slice(0, maxLength) : result);
}

const RANDOM_ESSAY = 'Random Essay';

class RandomEssay extends Component {
  static propTypes = {
...
export default RandomEssay;
export { RANDOM_ESSAY, randomEssayGenerator };
